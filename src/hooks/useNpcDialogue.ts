 ...
 */
export function useNpcDialogue(npcKeyArg: string | UseNpcDialogueOptions, options?: UseNpcDialogueOptions) {
  const opts: UseNpcDialogueOptions =
    typeof npcKeyArg === 'string' ? { ...(options ?? {} as UseNpcDialogueOptions), npcKey: npcKeyArg } : npcKeyArg;
  return useNpcDialogueImpl(opts);
}

function useNpcDialogueImpl({
  npcKey = 'bernard',
  user,
  currentLevel = 1,
  socialStat = 0,
  ...
  const advanceBernardStage = useCallback(
    async (stage: number) => {
      if (!user) return;
      await setFlag(user.id, stageFlagKey, String(stage));
      bumpFlags();
    },
    [user, bumpFlags, stageFlagKey],
  );
