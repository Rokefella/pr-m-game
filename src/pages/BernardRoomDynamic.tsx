...
const LightCell = memo(() => (
  <svg width="20" height="20" viewBox="0 0 20 20" style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
    <defs>
      <radialGradient id="bernardLampGlow" cx="50%" cy="50%" r="50%">
...
const BernardRoomDynamic = () => {
  ...
  // Player stats — feed the shared Bernard hook exactly as the village does.
  ...
  // Load Bernard's room layout for the current level
  useEffect(() => {
    ...
    const loadRoom = async () => {
      ...
      try {
        ...
        console.warn('[BernardRoomDynamic] layout load failed', e);
      }
    };
  ...
  const bernardPos = useMemo(() => {
    if (room?.bernardCenter) return room.bernardCenter;
    return { x: mapSize.w / 2, y: mapSize.h / 2 };
  }, [room, mapSize]);

  // Movement + collision — same approach as the library room
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#04040a', overflow: 'hidden' }}>
      ...
      {!roomLoading && room && (
        ...
        >
          ...
          {/* Bernard marker — same asset & placement parsing as the village */}
          <img
      ...
      <Thumbstick
        ...
        disabled={bernardOpen}
      />
    </div>
  );
};

export default BernardRoomDynamic;
